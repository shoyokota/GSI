...
!   compute vertical(pressure) interpolation index and weight
    ...
    enddo
    do k=1,msig
       if(sigma_avn(k)<tiny_single) sigma_avn(k)=tiny_single
       rlsigo(k)=log(sigma_avn(k))
    enddo
...
!   Load agv wgv bv
    ...
    subroutine berror_read_wgt_reg_extra(msig,mlat,num_bins2d,corz,corp,hwll,hwllp,vz,rlsig,varq,qoption,varcw,cwoption,mype,unit)
      ...
      use obsmod, only: extra_var2num,extra_var3num,if_cs_staticB
      ...
      real(r_kind),dimension(mlat,nsig,num_bins2d),intent(inout)::varq
      real(r_kind),dimension(mlat,nsig,num_bins2d),intent(inout)::varcw
...
! Get control variable indexes
  ...
  if(nrf3_q>0 .and. qoption==2)then
     do i=1,num_bins2d
        do k=1,nsig
           do j=1,mlat
              varq(j,k,i)=min(max(real(corz(j,k,i,nrf3_q),r_kind),0.0015_r_kind),one)
              corz(j,k,i,nrf3_q)=one
           enddo
        enddo
     enddo
  endif
        ...
        do i=1,num_bins2d
           do k=1,nsig
              if (ges_prslavg(k)>15.0_r_kind) then
                 do j=1,mlat
                    varcw(j,k,i)=max(real(corz(j,k,i,nrf3_cw),r_kind),zero)
                    corz(j,k,i,nrf3_cw)=one
                 enddo
              end if
           enddo
        enddo

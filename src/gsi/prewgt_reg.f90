...
! Arrays used for temperature-dependent error variance when assimilation radar dbz obs
  if ( (.not. if_cs_staticB) .and. l_be_T_dep) then
...
! Read in background error stats and interpolate in vertical to that specified in namelist
  ...
  if(mype==0) then
     ...
     do n=1,nc3d
        do i=1,num_bins2d
           do k=1,nsig
              if(n==nrf3_q.and.qoption==2) then
                 write(6,'(a10,2i5,3es12.4)') cvars3d(n),i,k,varq(min(11,mlat),k,i),hwll(j,k,i,n),vz(k,j,i,n)
              else
                 write(6,'(a10,2i5,3es12.4)') cvars3d(n),i,k,corz(j,k,i,n),hwll(j,k,i,n),vz(k,j,i,n)
              end if
           end do
        end do
     end do
     ...
     do n=1,nc2d
        do i=1,num_bins2d
           write(6,'(a10,2i5,2es12.4)') cvars2d(n),i,0,corp(j,i,n),hwllp(j,i,n)
        end do
     end do
  end if
...
! the parameter values here.
  do i=1,nhscrf
     if ( (.not. if_cs_staticB) .and. (l_set_be_rw .or. l_set_be_dbz) ) hzscl(i) = one
     hzscl(i)=one/hzscl(i)
  end do

! apply scaling to vertical length scales.  
! note:  parameter vs needs to be inverted
  if ( (.not. if_cs_staticB) .and. (l_set_be_rw .or. l_set_be_dbz) ) vs = one
  vs=one/vs
  vz=vz*vs
...
!         radar
  if ( (.not. if_cs_staticB) .and. l_set_be_rw ) then
      do n=1,nc3d
          if (n==nrf3_q) then
...
!         and corz(bkgd err) of cloud hydrometers (qr/qs/qg) when using radar reflectivity observations
  if ( (.not. if_cs_staticB) .and. l_set_be_dbz ) then
      do n=1,nc3d
          if (n==nrf3_qr ) then
              if (mype==0) &
...
!           only initialization of alv in subroutine smoothzo
        ...
        do j=1,lat2
           do i=1,lon2
              ...
              do k=1,nsig
                 if( if_cs_staticB )then
                    if ( maxval(ges_mask(j,i,:)) > 90.0_r_kind )then
                       dssv(j,i,k,n)=dl1*dsv(i,k,3,l)+dl2*dsv(i,k,3,l2)
                    else if( minval(ges_mask(j,i,:)) < -90.0_r_kind )then

     ...
     do k=1,n_ens
        ...
        do kk=1,grd_loc%nsig
           ...
           do j=1,grd_loc%lat2
              ...
              enddo
           enddo
           do j=1,obj_mgbf(1)%mm
              do i=1,obj_mgbf(1)%nm
...
! Apply vertical smoother on each ensemble member
  if(regional.and.l_mgbf_loc) then
     allocate(hwork_mgbf(obj_mgbf(ig)%km_a_all,obj_mgbf(ig)%nm,obj_mgbf(ig)%mm))
     hwork_mgbf=zero
...
!$omp parallel do schedule(dynamic,1) private(k,kk,ii,i,j,hwork_tmp)
     do k=1,n_ens
        do kk=1,grd_loc%nsig
           ...
           enddo
        enddo
        if(.not.obj_mgbf(ig)%l_loc_vertical) call new_factorization_rf_z(a_en(k)%r3(ipnt)%q,iadvance,iback,ig)
     enddo
     deallocate(hwork_mgbf)
  else
     iadvance=2 ; iback=1
...
101 format(F8.1,3x,F8.3,F8.4,3x,F8.4)
   ...
   if ( regional ) then
      if ( l_mgbf_loc ) then
         nval_lenz_en = maxval( obj_mgbf(1:naensgrp)%km_all &
...
!$$$ end documentation block
  ...
  if (r_ensloccov4tim>=zero.and.r_ensloccov4tim<=one) then
     ensloccov4tim=r_ensloccov4tim
     ensloccov4tim(1)=one
  else
     write(6,*)'setup_ensgrp2aensgrp: wrong r_ensloccov4tim'
     call stop2(666)
  endif
  if (r_ensloccov4var>=zero.and.r_ensloccov4var<=one) then
     ...
     write(6,*)'setup_ensgrp2aensgrp: wrong r_ensloccov4var'
     call stop2(666)
  endif
  if (r_ensloccov4scl>=zero.and.r_ensloccov4scl<=one) then
